...
def get_point_cloud(obj_pose,cam_traj,projectionMatrix):
    ...
    for idx in range(num_frame):
        ...
        # print("pointcloud ray",pointcloud_frame)
        # point_cloud_viz(pointcloud_frame.reshape((-1,3)))

        # print("can_traj",transform,transform.shape)
        # # intrinsics = torch.from_numpy(projectionMatrix)
        # # print("proj shape",projectionMatrix.shape,projectionMatrix)
        # rotation_matrix = Rotation.from_quat(cam_pose[3:])
        # rotation_matrix = rotation_matrix.as_matrix()
        # transform = np.eye(4)
        # transform[:3, :3] = rotation_matrix
        # transform[:3, -1] = cam_pose[:3]

        # extrinsics = np.inv(projectionMatrix * transform) #* NDC_pos
        max_obj_idx = seg.max()
        # print("obj pose",type(obj_pose),obj_pose,obj_pose.shape)
        num_obj  = obj_pose.shape[0]
        obj_frame=  np.zeros((num_obj,1000,3))
        for obj in range(max_obj_idx-num_obj+1,max_obj_idx+1):
            ...
            obj_pt_selected  = point_cloud_scaling(obj_all[id,:])
            # zero-centered
            ...
            obj_frame[max_obj_idx-num_obj+1-obj,:] = obj_pt_selected-center
            print("after centered",obj_pt_selected.min(axis=0)-center,obj_pt_selected.max(axis=0)-center,center)
            # point_cloud_viz(obj_pt_selected)
        point_cloud_total.append(obj_frame)
