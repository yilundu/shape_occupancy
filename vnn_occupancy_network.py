...
class VNNOccNet(nn.Module):
    def __init__(self,
        ...
        self.decoder = DecoderInner(dim=3, z_dim=latent_dim, c_dim=0, hidden_size=latent_dim, leaky=True, sigmoid=sigmoid, return_features=return_features)


    def forward(self, input):
        out_dict = {}

        enc_in = input['depth_coords'] * 10
...
class DecoderInner(nn.Module):
    ...
    def __init__(self, dim=3, z_dim=128, c_dim=128,
                 hidden_size=128, leaky=False, return_features=False, sigmoid=True):
        super().__init__()
        self.z_dim = z_dim
        self.c_dim = c_dim
